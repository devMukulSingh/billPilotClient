...
const productApiSlice = splitApi.injectEndpoints({
    endpoints: (build) => ({
        ...
        getAllProducts: build.query<TApiResponse<TProduct[]>, { userId: string | null | undefined }>({
            query: (arg) => ({
        ...
        getSearchedProducts: build.query<TApiResponse<TProduct[]>, TBaseQueryArg & {name:string | null}>({
            query: (arg) => ({
        ...
        deleteProduct: build.mutation<{}, { id: string | null,userId:string | null | undefined }>({
            query: (arg) => ({
                url: `/${arg.userId}/product/${arg.id}`,
                method: "DELETE"
            }),
